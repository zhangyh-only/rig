...
2. **判断部分**（你来做，禁止凭模板伪造）：
   ...
   - **openspec（按需，别无条件铺 `openspec/`）**：openspec 是外部 CLI、rig 不自带。先判 `applies_when`——**预研/demo 项目标 N/A、根本不铺 `openspec/`**；需求驱动型项目才考虑。需要且 openspec **已装**（`npx --no-install openspec --version` 或 `command -v openspec`）→ 经我同意 `npx openspec init` 建 `openspec/`（**root 级是 openspec 的固有约定，不是放 `docs/`**）再拷 change 模板；**未装 → 别铺 `openspec/`**，改在报告里标缺（见下）。
   - **缺的外部件一律响亮标缺、绝不替我装**：openspec / superpowers / feature-spec 等外部依赖缺失时，报告里逐一列「缺什么 + 安装命令（如 openspec=`npm i -g openspec`）+ 影响哪个命令（如 `/rig:new-change`）」。要联网安装单独征我同意，不擅自装。
