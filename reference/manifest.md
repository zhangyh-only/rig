...
| codex-skill-and-command-surface `[ready]` | Codex 可发现 rig skill、rig-* 操作 skills，并有本地 plugin surface | test -L ~/.codex/skills/rig && test -L ~/.agents/skills/rig && test -f ~/.codex/skills/rig-init/SKILL.md && test -f ~/.agents/plugins/rig/.codex-plugin/plugin.json && jq '.plugins[]? \| select(.name=="rig" and .policy.authentication=="ON_USE")' ~/.agents/plugins/marketplace.json | merge | scripts/install-codex-surface.sh 注册 skill 软链、生成 rig-* 操作 skills、安装 ~/.agents/plugins/rig、以 ON_USE 登记 marketplace；开新 Codex 会话后加载。项目换 AI 工具时仍需在该工具中跑 rig-init | true |
...
| agents-md-workflow-router-contract `[ready]` | AGENTS.md 含 Workflow Router 契约、多工具同步约束与交付说明约束，说明 Query / Fast Path / OpenSpec Change / OpenSpec + Implementation Plan / ADR / Review 的触发、边界、动作，并要求 Claude Code、Codex 等已声明适配层同步更新，收尾说明新项目与已接入 rig 项目的使用方式，防止 AI 过度流程化、误建 change、只改单一工具入口或改完不讲落地路径 | scripts/check-workflow-router.sh <proj> | merge | 补 `rig:workflow-router:start/end` 章节：小需求快路径、OpenSpec 与 implementation plan 分工、review 不创建 change、正反例；补多工具同步约束：Claude Code / Codex 必核对，未支持工具标“不适用 / 待补”；补交付说明约束：新项目怎么用、已接入 rig 项目怎么更新/检查 | true |
