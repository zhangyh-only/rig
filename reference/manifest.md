...
| feature-specs-dir-exists | docs/feature-specs/ 目录（L3 落点+memory-bank 迁移目标） | test -d docs/feature-specs | organize-existing | mkdir + 迁旧 memory-bank/feature-specs；新建用 feature-spec skill derive | true |
...
| memory-bank-legacy-not-migrated | 旧 memory-bank/ 迁到新结构 | ls memory-bank/conventions/ 或 .../feature-specs/ 非空且 docs/ 缺对应 | organize-existing | conventions→docs/conventions 打桶标、feature-specs→docs/feature-specs，保留原件加指针经许可清理 | false |
