#!/usr/bin/env bash
# 新设备一键 bootstrap：把全局机制接到已安装的 AI coding 工具（幂等）。在 rig/ 下跑：bash scripts/bootstrap.sh
set -u
...
bash "$here/scripts/backup.sh" "$HOME/.rig/hooks" "$HOME/.claude/hooks" "$HOME/.claude/agents" "$HOME/.claude/commands" "$HOME/.claude/settings.json" "$HOME/.claude/conventions.md" "$HOME/.codex/skills/rig" "$HOME/.agents/skills/rig" "$HOME/.agents/plugins/rig" "$HOME/.agents/plugins/marketplace.json" 2>/dev/null || true
echo "== 1. 机器画像 =="
...
if [ -d "$HOME/.codex" ] || command -v codex >/dev/null 2>&1 || [ -d "/Applications/Codex.app" ]; then
  echo "== 2a. 装 Codex 接线（检测到 Codex）=="
  if ! bash "$here/scripts/install-codex-hooks.sh"; then
    echo "  ✗ Codex hook 接线失败（见上），Claude Code 安装继续；修复后可重跑 bootstrap 或 rig init。" >&2
  fi
  if ! bash "$here/scripts/install-codex-surface.sh"; then
    echo "  ✗ Codex skill/command 接线失败（见上），Claude Code 安装继续；修复后可重跑 bootstrap 或 rig init。" >&2
  fi
else
  echo "== 2a. Codex 接线 =="
...
echo "开新会话使 Claude hook 与 /rig: 命令生效；Codex 已注册 skill 与 /rig:* command surface，通常需重启/开新 Codex 会话后在该项目执行 /rig:init；Codex CLI 如提示 hook 待信任，用 /hooks review + trust。建议把 ~/.rig/hooks、~/.agents/plugins/rig 与 ~/.claude/{agents,commands,settings.json,conventions.md} 纳入 dotfiles 仓库（用 assets/dotfiles-layer/claude-dotfiles.gitignore 挡机密）。"
