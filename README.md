...
- Codex：`~/.codex/hooks -> ~/.rig/hooks`、`~/.codex/hooks.json`、`~/.codex/skills/rig`、`~/.agents/skills/rig`、`~/.codex/skills/rig-*` / `~/.agents/skills/rig-*` 操作 skills、`~/.agents/plugins/rig`（本地插件用 `ON_USE`，不触发连接授权）；Codex CLI 用 `/hooks` 管理 hook trust，Desktop App 当前普通会话不支持该 slash command
