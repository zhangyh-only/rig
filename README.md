...
- Codex：`~/.codex/hooks -> ~/.rig/hooks`、`~/.codex/hooks.json`、`~/.codex/skills/rig`、`~/.agents/skills/rig`、`~/.agents/plugins/rig`（提供 Codex 侧 skill 与 `/rig:init`/`/rig:doctor` command surface）；Codex CLI 用 `/hooks` 管理 hook trust，Desktop App 当前普通会话不支持该 slash command
...
`bootstrap.sh` 和 `rig init` 都会 **auto 检测本机已装工具** 并补齐适配：Claude Code 走完整 hook/命令/skill 机制；Codex 会写 `~/.codex/hooks.json`，注册 `~/.codex/skills/rig` / `~/.agents/skills/rig`，并安装本地 plugin command surface（`~/.agents/plugins/rig`），让 Codex 会话能发现 rig skill 并执行 `/rig:init` / `/rig:doctor`。`--claude` / `--codex` / `--cursor` 仅用于限定 `rig init` 本次只处理某个工具。Codex CLI 如提示 hook 待信任，用 `/hooks` review + trust；Codex Desktop App 当前普通会话不支持 `/hooks`。**Cursor** 仍仅 canonical + CI 兜底。
