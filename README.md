...
```
- 跑完**开一个新 Claude 会话**（hooks 与 `/rig:*` 命令在新会话才生效）；Codex CLI 如提示 hook 待信任，用 `/hooks` review + trust。Codex Desktop App 当前普通会话没有 `/hooks` 入口，按客户端实际提示处理。
- 克隆目录别移走（skill 软链指向它）。
...
2. 敲 **`/rig:init`**（或终端跑 `rig init`）—— AI/CLI 检测缺啥补啥：本机检测已装 Claude/Codex/Cursor 并补对应工具接线；项目里铺骨架、把既有规范（`.cursorrules`/copilot-instructions…）归并进 `docs/conventions/` 三桶、从 `pom`/`package.json` 推导 `AGENTS.md` 地图、起草 `scripts/verify-local.sh`。
3. 审 diff，确认后 commit。
...
- Codex：`~/.codex/hooks -> ~/.rig/hooks`、`~/.codex/hooks.json`；Codex CLI 用 `/hooks` 管理 trust，Desktop App 当前普通会话不支持该 slash command
...
bash test/codex-hooks.sh  # 验证 Codex hook JSON 输出、rig init auto 多工具接线与幂等注册
...
`bootstrap.sh` 和 `rig init` 都会 **auto 检测本机已装工具** 并补齐适配：Claude Code 走完整 hook/命令/skill 机制；Codex 已接入最小闭环，会写 `~/.codex/hooks.json`，让 `UserPromptSubmit` 注入规范、`PostToolUse` 在改完文件后跑 lint 并用 `decision:block` 回灌。`--claude` / `--codex` / `--cursor` 仅用于限定 `rig init` 本次只处理某个工具。Codex CLI 如提示 hook 待信任，用 `/hooks` review + trust；Codex Desktop App 当前普通会话不支持 `/hooks`。**Cursor** 仍仅 canonical + CI 兜底。
