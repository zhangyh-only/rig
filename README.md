...
## Workflow Router 如何落地到项目

- **新项目**：按上面的 B 步执行 `/rig:init`。新生成的 `AGENTS.md` 会自带 Workflow Router、多工具同步约束和交付说明约束；完成后跑 `rig doctor "$PWD"` 或 `/rig:doctor`，确认第 7 段 Workflow Router 契约通过。
- **已接入 rig 的项目**：重新运行 `/rig:init` 不会覆盖已有 `AGENTS.md`，所以需要把 `assets/project-layer/AGENTS.md` 中 `rig:workflow-router:start/end` 包围的章节，以及“多工具同步约束 / 交付说明约束”手动合并进项目 `AGENTS.md`；再跑 `rig doctor "$PWD"` 检查缺口。
- **工具侧入口**：Claude Code 的 `.claude/commands/rig/*.md` 和 Codex 的 action skills / plugin command surface 都已同步新契约。已接入项目若已有旧 `.claude/commands/rig/*.md`，`cp -Rn` 不会覆盖它们，需要按需手动合并或删除后重新 `/rig:init`。
- **会话生效**：hook、skill、command surface 变更通常需要打开新的 AI 工具会话。Codex CLI 如提示 hook trust，用 `/hooks` review + trust；Claude Code 侧确认 `~/.claude` 命令和 hook 已加载。
