...
- **已接入 rig 的项目**：重新运行 `/rig:init` 或 `rig init <项目路径>`。rig 会自动补齐已有 `AGENTS.md` 里缺失的 Workflow Router、多工具同步约束和交付说明约束；已有内容保留，不需要复制文件。补齐后跑 `rig doctor "$PWD"` 检查缺口。
- **工具侧入口**：Claude Code 的 `.claude/commands/rig/*.md` 和 Codex 的 action skills / plugin command surface 都已同步新契约。已接入项目若已有旧 `.claude/commands/rig/*.md`，重新运行 `/rig:init` 会自动更新 frontmatter description 并补齐“路由边界”段，同时保留已有命令正文。
